...
class hexapod:
    ...
    def __init__(self, init_viz: bool = True, logging_level: int = logging.DEBUG) -> None:
        ...
        self.pin_model_dir = str(
            Path('./meshcat_sim/URDF/Physics_Model_Edited').absolute())
        ...
        self.HALF_STEP_SIZE_XY = 0.0/2  # Half of the step size in XY plane
    ...
    def init_foot_trajectory_functions(self, step_size_xy_mult: float, DIR: str = 'N', FOOT_ID: int = 10) -> None:
        ...
        p2 = np.append(p2, p1[2])
         ...
         # Apex points (midway)
        y_apex = max(p1[1], p2[1]) + (self.XY_CLEARANCE * np.sign(max(p1[1], p2[1])))
        z_apex = max(p1[2], p2[2]) + (self.Z_CLEARANCE * np.sign(max(p1[2], p2[2])))

        # ---- NEW: set up times and heights for Hermite spline ----
        ts = np.array([0.0, 0.5, 1.0])
        zs = np.array([p1[2], z_apex, p2[2]])

        # Enforce zero slope (lift-off & landing)
        dz0 = 0.0
        dz2 = 0.0

        # For the midpoint slope, approximate it so the spline is well‑behaved
        dz1 = (zs[2] - zs[0]) / (ts[2] - ts[0])

        # Build the cubic Hermite spline
        spline = CubicHermiteSpline(ts, zs, [dz0, dz1, dz2])

        # Redefine your trajectory functions
        self.x_t = lambda t: (1 - t) * p1[0] + t * p2[0]
        self.y_t = lambda t: np.interp(t, ts, [p1[1], y_apex, p2[1]])  # you can keep your parabola or similarly spline y if desired
        self.z_t = lambda t: spline(t)
    ...
    def generate_joint_waypoints(self, step_size_xy_mult: float, WAYPOINTS: int = 5, DIR: str = 'N', FOOT_ID: int = 10) -> List[np.ndarray]:
        """
        Generate joint waypoints for a foot trajectory using Hermite‐spline z(t).
        """
        # Initialize foot trajectory functions
        self.init_foot_trajectory_functions(step_size_xy_mult=step_size_xy_mult, DIR=DIR, FOOT_ID=FOOT_ID)
        s = np.linspace(0, 1, WAYPOINTS)

        # Build (x,y,z) waypoints, extracting scalars from spline arrays
        waypoints = []
        for t in s:
            x = self.x_t(t)
            y = self.y_t(t)
            z_arr = self.z_t(t)
            z = float(z_arr) if isinstance(z_arr, np.ndarray) else z_arr
            waypoints.append([round(x, 5), round(y, 5), round(z, 5)])

        # Inverse‐geometry for each foot waypoint
        return [
            self.inverse_geometery(q=self.qc, FRAME_ID=FOOT_ID, desired_pos=wp)
            for wp in waypoints
        ]

    def generate_waypoints(self, step_size_xy_mult: float, WAYPOINTS: int = 5, DIR: str = 'N', leg_set: int = 0):
        """
        Generate full body+leg waypoints for a tripod leg_set (0 or 1).
        """
        if leg_set not in (0, 1):
            raise ValueError(f'leg_set must be 0 or 1, got {leg_set}')
    
        # start with current state repeated
        waypoints = self.state_c @ np.ones((1, WAYPOINTS))
    
        # Swing each foot in the set
        for foot in self.FOOT_IDS[leg_set::2]:
            self.init_foot_trajectory_functions(step_size_xy_mult=step_size_xy_mult, DIR=DIR, FOOT_ID=foot)
            s = np.linspace(0, 1, WAYPOINTS)
    
            # build foot xyz waypoints
            foot_xyz = []
            for t in s:
                x = self.x_t(t)
                y = self.y_t(t)
                z_arr = self.z_t(t)
                z = float(z_arr) if isinstance(z_arr, np.ndarray) else z_arr
                foot_xyz.append([round(x, 5), round(y, 5), round(z, 5)])
            foot_wp = np.array(foot_xyz).T
    
            # place into the big waypoint matrix
            idx = 3 + 3 * (self.FOOT_IDS.index(foot))
            waypoints[idx:idx+3, :] = foot_wp
    
        # Now body translation (z stays constant)
        self.init_body_trajectory_functions(step_size_xy_mult=step_size_xy_mult, DIR=DIR)
        s = np.linspace(0, 1, WAYPOINTS)
    
        body_xyz = []
        z_body = float(self.state_c[2, 0])
        for t in s:
            x = self.x_t(t)
            y = self.y_t(t)
            body_xyz.append([round(x, 5), round(y, 5), round(z_body, 5)])
        body_wp = np.array(body_xyz).T
    
        waypoints[0:3, :] = body_wp
        return waypoints
...
if __name__ == "__main__":
    ...
    hexy = hexapod(init_viz=True, logging_level=logging.CRITICAL)
    ...
    for trials in range(4):
        ...
        wp = hexy.generate_waypoints(
            WAYPOINTS=WAYPOINTS,
            step_size_xy_mult=step_size_xy_mult,
            leg_set=leg_set
        )
        
        # hexy.plot_trajctory(state=wp.T, title=f"Waypoints for Trial {trials}")

        # wp = [wp[:, i].reshape(-1, 1) for i in range(wp.shape[1])]  # Convert to list of 21x1 arrays

        # wp_arr = wp  # wp is (21 x N)
        # fig = plt.figure(figsize=(10, 6))
        # ax = fig.add_subplot(111, projection='3d')

        # # Plot base trajectory
        # ax.plot(wp_arr[0, :], wp_arr[1, :], wp_arr[2, :], label="Base", color='black')

        # # Plot each leg trajectory
        # for i in range(6):
        #     xi = wp_arr[3 + 3*i, :]
        #     yi = wp_arr[4 + 3*i, :]
        #     zi = wp_arr[5 + 3*i, :]
        #     ax.plot(xi, yi, zi, label=f"Leg {i}")

        # ax.set_title("Hexapod Trajectory (Trial {})".format(trials))
        # ax.set_xlabel('X')
        # ax.set_ylabel('Y')
        # ax.set_zlabel('Z')
        # ax.legend()
        ...
        # plt.show()
        print(wp)
        wp = [wp[:, i].reshape(-1, 1) for i in range(wp.shape[1])] # Unpacking wp whic is a 21XN statespace to 21X1 lists
    ...
    # filename = f'hexapod_trajectory_NE_{strftime("%Y%m%d_%H%M%S")}.npy'
    # save_path = f'/home/jithin/Desktop/EXPO/hexapod/meshcat_sim/trajectories/{filename}'
    # np.save(save_path, q)
