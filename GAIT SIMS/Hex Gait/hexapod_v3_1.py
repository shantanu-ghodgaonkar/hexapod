...
class hexapod:
    ...
    def __init__(self, init_viz=True, logging_level=logging.DEBUG) -> None:
        ...
        self.pin_model_dir = str(
            Path('./URDF/VREP_URDF').absolute())
        self.urdf_filename = (self.pin_model_dir + '/URDF4DynamicsSim.urdf')
        ...
        self.FOOT_IDS = [self.robot.model.getFrameId(
            frame.name) for frame in self.robot.model.frames if "endeff" in frame.name]
...
if __name__ == "__main__":
    ...
    pick_up_leg = [0., 0., hexy.STEP_SIZE_Z]

    l0 = hexy.inverse_geometery(q=hexy.robot.q0, FRAME_ID=hexy.FOOT_IDS[0], desired_pos=(
        hexy.robot.framePlacement(hexy.robot.q0, hexy.FOOT_IDS[0]).translation + pick_up_leg))
    l2 = hexy.inverse_geometery(q=hexy.robot.q0, FRAME_ID=hexy.FOOT_IDS[2], desired_pos=(
        hexy.robot.framePlacement(hexy.robot.q0, hexy.FOOT_IDS[2]).translation + pick_up_leg))
